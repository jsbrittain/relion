...
#ifdef TIMING
int TIMING_MPIPACK, TIMING_MPIWAIT, TIMING_MPICOMBINEDISC,
    TIMING_MPICOMBINENETW, TIMING_MPISLAVEWORK;
    ...
    if (!node->isLeader()) {
      ...
      for (int i = 0; i < devCount; i++) {
        ...
        if (deviceProp.major > CUDA_CC_MAJOR)
          compatibleDevices += 1;
        else if (deviceProp.major == CUDA_CC_MAJOR &&
                 deviceProp.minor >= CUDA_CC_MINOR)
          compatibleDevices += 1;
#elif _HIP_ENABLED
        compatibleDevices += 1;
#endif
...
will still yield good performance and possibly a more stable execution. \n"
          ...
          std::cerr
              << "You are also using double precison on the GPU. If you were using single precision\n\
(which in all tested cases is perfectly fine), then you could use an box-size of ~"
              << sLowBoxLim << "." << std::endl;
#endif
      ...
      for (int i = 0; i < nr_threads; i++) {
        ...
        if (semiAutomaticMapping) {
          if (fullAutomaticMapping) {
            ...
            if (nodeSize > 1)
              dev_id = (devCount *
                        (((node->rank - 1) % nodeSize) * nr_threads + i)) /
                       (nodeSize * nr_threads);
            else
    ...
    for (int follower = 1; follower < node->size; follower += 2) {
      ...
      if (node->isLeader())
        ...
        node->relion_MPI_Recv(&mymodel.nr_particles_per_group[0],
                              mymodel.nr_particles_per_group.size(), MPI_LONG,
                              0, MPITAG_METADATA, MPI_COMM_WORLD, status);
    }
#endif
...
void MlOptimiserMpi::setupAccelerators() {
  ...
  if (do_gpu && !node->isLeader()) {
    ...
    for (int i = 0; i < gpuOptimiserDeviceMap.size(); i++) {
      ...
      MlOptimiserAccGPU *b = new MlOptimiserAccGPU(
          this, (MlDeviceBundle *)accDataBundles[gpuOptimiserDeviceMap[i]],
          didSs.str().c_str());
      b->resetData();
    ...
    for (int i = 0; i < accDataBundles.size(); i++) {
      if (((MlDeviceBundle *)accDataBundles[i])->device_id >= devCount ||
          ((MlDeviceBundle *)accDataBundles[i])->device_id < 0) {
        // std::cerr << " using device_id=" <<
        // ((MlDeviceBundle*)accDataBundles[i])->device_id << " (device no. " <<
        // ((MlDeviceBundle*)accDataBundles[i])->device_id+1 << ") which is not
        // within the available device range" << devCount << std::endl;
        CRITICAL(ERR_GPUID);
      } else
        HANDLE_ERROR(
            accGPUSetDevice(((MlDeviceBundle *)accDataBundles[i])->device_id));
      ...
      size_t required_free =
          ...
          GPU_THREAD_MEMORY_OVERHEAD_MB * 1000 * 1000 * threadcountOnDevice[i];
      ...
      if (free < required_free) {
        ...
        // #ifdef DEBUG_HIP
        // 	printf("WARNING: due to 296623 ticket, use 50%% of free GPU
        // memory as allocationSize %zu MB.\n", allocationSize/1000000); #else
        // 	printf("WARNING: Using 50%% of free GPU memory as allocationSize
        // %zu MB.\n", allocationSize/1000000); #endif
#endif
      }
      ...
      printf("INFO: Projector model size %dx%dx%d\n",
             (int)mymodel.PPref[0].data.xdim, (int)mymodel.PPref[0].data.ydim,
             (int)mymodel.PPref[0].data.zdim);
      printf("INFO: Free memory for Custom Allocator of device bundle %d of "
             "rank %d is %d MB\n",
  ...
  if (do_gpu && !node->isLeader()) {
    for (int i = 0; i < accDataBundles.size(); i++)
      ((MlDeviceBundle *)accDataBundles[i])
          ->setupTunableSizedObjects(allocationSizes[i]);
  }
#endif // _CUDA_ENABLED or _HIP_ENABLED
#ifdef _SYCL_ENABLED
  if (do_sycl && !node->isLeader()) {
    ...
    std::transform(
        strStream.begin(), strStream.end(), strStream.begin(),
        [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
    const bool isStream =
        (strStream == "1" || strStream == "on") ? true : false;

    for (int i = 0; i < gpuDevices.size(); i++) {
      accDataBundles.push_back(
          (void *)(new MlSyclDataBundle(syclDeviceList[gpuDevices[i]])));
      ((MlSyclDataBundle *)accDataBundles[i])->setup(this);
    }

    for (int i = 0; i < gpuOptimiserDeviceMap.size(); i++) {
      MlOptimiserSYCL *b = new MlOptimiserSYCL(
          this, (MlSyclDataBundle *)accDataBundles[gpuOptimiserDeviceMap[i]],
          isStream, "sycl_optimiser");
      b->resetData();
      b->threadID = i;
      gpuOptimisers.push_back((void *)b);
    }
  ...
  if (do_cpu && !node->isLeader()) {
    ...
    if (posix_memalign((void **)&mdlClassComplex, MEM_ALIGN,
...
// #define DEBUG_MPIEXP2
      ...
      std::cerr << " MASTER RECEIVING from follower= " << this_follower
                ...
                << " JOB_NIMG= " << job_nimg
                << " JOB_NPAR= " << (job_last - job_first + 1) << std::endl;
#endif
      ...
      //		" JOB_FIRST= " << job_first << " JOB_LAST= " << job_last << std::endl;
...
void MlOptimiserMpi::runFollowerExpectationLoop(
  ...
  try {
    ...
    while (true) {
      ...
      if (job_nimg <= 0) {
#ifdef DEBUG
        std::cerr << " follower " << node->rank << " has finished expectation.."
                  << std::endl;
#endif
        exp_imagedata.clear();
        exp_metadata.clear();
...
#ifdef DEBUG_MPIEXP
        std::cerr << " SLAVE EXECUTING node->rank= " << node->rank
                  << " JOB_FIRST= " << job_first << " JOB_LAST= " << job_last
                  << std::endl;
#endif
    ...
    if (do_gpu) {
      ...
      for (int i = 0; i < accDataBundles.size(); i++) {
        ...
        if (((MlDeviceBundle *)accDataBundles[i])
                ->allocator->getNumberOfAllocs() != 0) {
          printf("DEBUG_ERROR: Non-zero allocation count encountered in custom "
    ...
    if (do_sycl) {
      for (int i = 0; i < accDataBundles.size(); i++) {
        ...
        for (int j = 0; j < b->backprojectors.size(); j++) {
          ...
          XFLOAT *reals = (XFLOAT *)(stream->syclMalloc(s * sizeof(XFLOAT),
                                                        syclMallocType::host));
          XFLOAT *imags = (XFLOAT *)(stream->syclMalloc(s * sizeof(XFLOAT),
                                                        syclMallocType::host));
          XFLOAT *weights = (XFLOAT *)(stream->syclMalloc(
              s * sizeof(XFLOAT), syclMallocType::host));
...
#ifdef DEBUG
      if (node->rank == 1)
        std::cerr << " MPI_Allreduce MULTIDIM_SIZE(Mpack)= "
    ...
    while (piece < nr_pieces) {
        ...
        if (other_follower < node->size) {
          if (node->rank == this_follower) {
#ifdef DEBUG
            std::cerr << " AA SEND node->rank= " << node->rank
            ...
            node->relion_MPI_Recv(MULTIDIM_ARRAY(Msum), MULTIDIM_SIZE(Msum),
                                  ...
                                  MPI_COMM_WORLD, status);
#ifdef DEBUG
            std::cerr << " AA RECV node->rank= " << node->rank
                      << " MULTIDIM_SIZE(Msum)= " << MULTIDIM_SIZE(Msum)
                      << " this_follower= " << this_follower
                      << " other_follower= " << other_follower << std::endl;
#endif
          ...
          if (node->rank == this_follower) {
#ifdef DEBUG
            std::cerr << " BB SEND node->rank= " << node->rank
                      << " MULTIDIM_SIZE(Msum)= " << MULTIDIM_SIZE(Msum)
                      << " this_follower= " << this_follower
            ...
            node->relion_MPI_Recv(MULTIDIM_ARRAY(Msum), MULTIDIM_SIZE(Msum),
                                  ...
                                  MPI_COMM_WORLD, status);
#ifdef DEBUG
            std::cerr << " BB RECV node->rank= " << node->rank
                      << " MULTIDIM_SIZE(Msum)= " << MULTIDIM_SIZE(Msum)
                      << " this_follower= " << this_follower
                      << " first_follower= " << first_follower << std::endl;
#endif
      ...
      for (int this_follower = 1; this_follower < node->size; this_follower++) {
        ...
        if (other_follower < node->size - nr_halfsets) {
          if (node->rank == this_follower) {
#ifdef DEBUG
            std::cerr << " CC SEND node->rank= " << node->rank
                      << " MULTIDIM_SIZE(Msum)= " << MULTIDIM_SIZE(Msum)
                      << " this_follower= " << this_follower
                      << " other_follower= " << other_follower << std::endl;
#endif
            ...
            node->relion_MPI_Recv(MULTIDIM_ARRAY(Msum), MULTIDIM_SIZE(Msum),
                                  ...
                                  MPI_COMM_WORLD, status);
#ifdef DEBUG
            std::cerr << " CC RECV node->rank= " << node->rank
                      << " MULTIDIM_SIZE(Msum)= " << MULTIDIM_SIZE(Msum)
                      << " this_follower= " << this_follower
                      << " other_follower= " << other_follower << std::endl;
#endif
      ...
      if (do_split_random_halves) {
        if (!do_join_random_halves) {
          ...
          if (!node->isLeader()) {
            ...
            int reconstruct_rank = 2 * (ith_recons % ((node->size - 1) / 2)) +
                                   node->myRandomSubset();
            ...
            //MULTIDIM_SIZE(mymodel.fsc_halves_class[ibody]), MY_MPI_DOUBLE,
            //split_rank, node->splitC);
          }
#else
          MPI_Status status;
          // Make sure I am sending from the rank where the reconstruction was
          // done (see above) to all other followers of this subset Loop twice
          // through this, as each class was reconstructed by two different
          // followers!!
          int nr_halfsets = 2;
              ...
              for (int recv_node = my_first_recv; recv_node < node->size;
                ...
                if (node->rank == reconstruct_rank && recv_node != node->rank) {
#ifdef DEBUG
                  std::cerr << "ihalfset= " << ihalfset
                            << " Sending iclass=" << iclass
                            << " Sending ibody=" << ibody << " from node "
                            << reconstruct_rank << " to node " << recv_node
                            << std::endl;
#endif
                  node->relion_MPI_Send(
                      MULTIDIM_ARRAY(mymodel.Iref[ith_recons]),
                      MULTIDIM_SIZE(mymodel.Iref[ith_recons]), MY_MPI_DOUBLE,
                      recv_node, MPITAG_IMAGE, MPI_COMM_WORLD);

                  if (do_grad) {
                    node->relion_MPI_Send(
                        MULTIDIM_ARRAY(mymodel.Igrad1[ith_recons]),
                        MULTIDIM_SIZE(mymodel.Igrad1[ith_recons]),
                        MY_MPI_DOUBLE, recv_node, MPITAG_IMAGE, MPI_COMM_WORLD);
                    node->relion_MPI_Send(
                        MULTIDIM_ARRAY(mymodel.Igrad2[ith_recons]),
                        MULTIDIM_SIZE(mymodel.Igrad2[ith_recons]),
                        MY_MPI_DOUBLE, recv_node, MPITAG_IMAGE, MPI_COMM_WORLD);
                  }
                  ...
                  node->relion_MPI_Send(
                      MULTIDIM_ARRAY(mymodel.data_vs_prior_class[ith_recons]),
                      MULTIDIM_SIZE(mymodel.data_vs_prior_class[ith_recons]),
                      MY_MPI_DOUBLE, recv_node, MPITAG_METADATA,
                      MPI_COMM_WORLD);
                  node->relion_MPI_Send(
                      MULTIDIM_ARRAY(
                          mymodel.fourier_coverage_class[ith_recons]),
                  ...
                  node->relion_MPI_Send(
                      ...
                      MY_MPI_DOUBLE, recv_node, MPITAG_RFLOAT, MPI_COMM_WORLD);
                  // node->relion_MPI_Send(MULTIDIM_ARRAY(mymodel.fsc_halves_class[ibody]),
                  // MULTIDIM_SIZE(mymodel.fsc_halves_class[ibody]),
                  // MY_MPI_DOUBLE, recv_node, MPITAG_RANDOMSEED,
                  // MPI_COMM_WORLD);
                } else if (node->rank != reconstruct_rank &&
                           node->rank == recv_node) {
                  ...
                  node->relion_MPI_Recv(
                      MULTIDIM_ARRAY(mymodel.data_vs_prior_class[ith_recons]),
                      MULTIDIM_SIZE(mymodel.data_vs_prior_class[ith_recons]),
                      MY_MPI_DOUBLE, reconstruct_rank, MPITAG_METADATA,
                      MPI_COMM_WORLD, status);
                  node->relion_MPI_Recv(
                      MULTIDIM_ARRAY(
                          mymodel.fourier_coverage_class[ith_recons]),
                  ...
                  node->relion_MPI_Recv(
                      MULTIDIM_ARRAY(mymodel.sigma2_class[ith_recons]),
                      MULTIDIM_SIZE(mymodel.sigma2_class[ith_recons]),
                      MY_MPI_DOUBLE, reconstruct_rank, MPITAG_RFLOAT,
                      MPI_COMM_WORLD, status);
                  // node->relion_MPI_Recv(MULTIDIM_ARRAY(mymodel.fsc_halves_class[ibody]),
                  // MULTIDIM_SIZE(mymodel.fsc_halves_class[ibody]),
                  // MY_MPI_DOUBLE, reconstruct_rank, MPITAG_RANDOMSEED,
                  // MPI_COMM_WORLD, status);
#ifdef DEBUG
                  std::cerr << "ihalfset= " << ihalfset
                            << " Received!!!=" << iclass << " ibody=" << ibody
                            << " from node " << reconstruct_rank << " at node "
                            << node->rank << std::endl;
#endif
                }
              }
...
void MlOptimiserMpi::joinTwoHalvesAtLowResolution() {
#ifdef DEBUG
  std::cerr << "MlOptimiserMpi::joinTwoHalvesAtLowResolution: Entering "
            << std::endl;
#endif
  ...
  for (int ibody = 0; ibody < mymodel.nr_bodies; ibody++) {
#ifdef DEBUG
    std::cerr << " ibody= " << ibody << " node->rank= " << node->rank
...
#ifdef DEBUG
  std::cerr << "MlOptimiserMpi::joinTwoHalvesAtLowResolution: done"
            << std::endl;
#endif
}
