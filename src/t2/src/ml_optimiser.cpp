...
// #include "src/apps/pinned_allocator/pinned_allocator.cuh"
...
void globalThreadExpectationSomeParticles(void *self, int thread_id,int task_limit)
{
...
#ifdef _CUDA_ENABLED
		if (MLO->do_gpu)
			((MlOptimiserCuda*) MLO->cudaOptimisers[thread_id])->doThreadExpectationSomeParticles(thread_id,task_limit);
		else
#endif
	...
	catch (RelionError XE)
	{
		...
		std::cerr << XE<<std::flush;
	}
	// printf("Thread %d finished expectation\n", thread_id);
	// fflush(stdout);
}
...
//#define DEBUG_READ
	...
	if (checkParameter(argc, argv, "--xjl"))
	{
		xjl_type=1;
	}
	else
	...
	task_limit = textToInteger(parser.getOption("--task_limit", "Number of parallel task for each thread", "1"));
...
void MlOptimiser::parseContinue(int argc, char **argv)
{
	...
	x_pool = textToInteger(parser.getOption("--pool", "Number of images to pool for each thread task", "4"));
...
void MlOptimiser::parseInitial(int argc, char **argv)
{
	...
	int computation_section = parser.addSection("Computation");
	x_pool = textToInteger(parser.getOption("--pool", "Number of images to pool for each thread task", "100"));
	nr_threads = textToInteger(parser.getOption("--j", "Number of threads to run in parallel (only useful on multi-core machines)", "1"));
	combine_weights_thru_disc = !parser.checkOption("--dont_combine_weights_via_disc", "Send the large arrays of summed weights through the MPI network, instead of writing large files to disc");
	do_shifts_onthefly = parser.checkOption("--onthefly_shifts", "Calculate shifted images on-the-fly, do not store precalculated ones in memory");
	do_parallel_disc_io = !parser.checkOption("--no_parallel_disc_io", "Do NOT let parallel (MPI) processes access the disc simultaneously (use this option with NFS)");
...
void MlOptimiser::initialiseGeneral(int rank)
{
	...
	TIMING_EXTRA4= timer.setNew(" -extra4");

	// for(int i=0;i<nr_threads;i++)
	// {
	// 	TIMING_EXTRA1_T[i]= timer.setNew(" -extra1_t"+integerToString(i));
	// 	TIMING_EXTRA2_T[i]= timer.setNew(" -extra2_t"+integerToString(i));
	// 	TIMING_EXTRA3_T[i]= timer.setNew(" -extra3_t"+integerToString(i));
	// 	TIMING_EXTRA4_T[i]= timer.setNew(" -extra4_t"+integerToString(i));
...
//#define DEBUG_EXP
...
#endif
			// ((MlDeviceBundle*)accDataBundles[i])->setupTunableSizedObjects(allocationSize,threadcountOnDevice[i]);
	...
	{
		// my_first_part_id=0;
		// nr_particles_done=1000;
		// my_nr_particles=1735;
		// my_last_part_id=1735;
		// XXX: 
		my_nr_particles=200;
		my_last_part_id=199;
	}
	...
	while (nr_particles_done < my_nr_particles)
	{
		...
		// printf("my_pool_first_part_id=%ld, my_pool_last_part_id=%ld\n", my_pool_first_part_id, my_pool_last_part_id);
		expectationSomeParticles(my_pool_first_part_id, my_pool_last_part_id);
		// std::cout<<"expectationSomeParticles"<<std::endl;
#ifdef TIMING
		...
		// std::cout<<"monitorHiddenVariableChanges"<<std::endl;
#ifdef TIMING
		...
		// std::cout<<"setMetaDataSubset"<<std::endl;
#ifdef TIMING
		timer.toc(TIMING_EXP_METADATA);
#endif
	...
	if (do_gpu)
	{
		...
		for (int i = 0; i < accDataBundles.size(); i ++)
		{
			...
			if (((MlDeviceBundle*) accDataBundles[i])->allocator->getNumberOfAllocs() != 0)
			{
				...
				// std::cout<<"clear accDataBundles0"<<std::endl;

		for (int i = 0; i < accDataBundles.size(); i ++)
			delete (MlDeviceBundle*) accDataBundles[i];

		accDataBundles.clear();
				// std::cout<<"clear accDataBundles"<<std::endl;
...
//#define DEBUG_EXPSOME
	...
	auto part_id_sorted_count = my_last_part_id - my_first_part_id + 1;
	std::vector<int> prefix_sum_metadata_offset(part_id_sorted_count, 0);

	// std::vector<int> prefix_sum_metadata_offset_test(part_id_sorted_count, 0);
	std::vector<long> local_sums(nr_threads, 0);
	
	// for (int i = 1; i < part_id_sorted_count; i ++) {
	// 	prefix_sum_metadata_offset[i] = prefix_sum_metadata_offset[i-1]
	// 	 	+ mydata.numberOfImagesInParticle(mydata.sorted_idx[i + my_first_part_id - 1]);
	// }
    ...
    #pragma omp parallel num_threads(nr_threads)
    {
		...
		long block_size = (part_id_sorted_count + num_threads - 1) / num_threads;
		
		long start = std::min(tid * block_size, part_id_sorted_count);
		long end = std::min((tid + 1) * block_size, part_id_sorted_count);

        long local_sum = 0;
		for (long i = start; i < end; i ++) {
			prefix_sum_metadata_offset[i] = local_sum;
			local_sum += mydata.numberOfImagesInParticle(mydata.sorted_idx[i + my_first_part_id]);
		}
        ...
        for (long i = start; i < end; i ++) {
            prefix_sum_metadata_offset[i] += offset;
        }
    }

	// for (long i = 0; i < part_id_sorted_count; i ++) {
	// 	if (prefix_sum_metadata_offset[i] != prefix_sum_metadata_offset_test[i]) {
	// 		printf("ERROR! prefix_sum_metadata_offset[%6d]:%8d != prefix_sum_metadata_offset_test[%6d]:%8d\n",
	// 				i, prefix_sum_metadata_offset[i], i, prefix_sum_metadata_offset_test[i]);
	// 		fflush(stdout);
	// 	}
	// }
    // #pragma omp parallel num_threads(nr_threads)
    ...
    //     int start = tid * (part_id_sorted_count / num_threads) + 1;
    //     int end = (tid + 1) * (part_id_sorted_count / num_threads);
    //     if (tid == num_threads - 1) {
    //         end = part_id_sorted_count;
    //     }

    //     long local_sum = 0;
    //     for (int i = start; i <= end; ++i) {
    //         local_sum += mydata.numberOfImagesInParticle(mydata.sorted_idx[i + my_first_part_id - 1]);
    //         prefix_sum_metadata_offset[i] = local_sum;
    //     }
    //     local_sums[tid] = local_sum;
	...
	// resize
	if (do_only_sample_tilt) {
		sampling.psi_angles.clear();
		sampling.psi_angles.resize(part_id_sorted_count);
	}
	if (do_skip_align || do_skip_rotate) {
		sampling.resizeOrientation(part_id_sorted_count, true);
	}
	if (do_skip_align) {
		sampling.resizeTranslation(part_id_sorted_count, true);
	}
	...
	{
		auto metadata_offset = prefix_sum_metadata_offset[part_id_sorted - my_first_part_id];
    	long int part_id = mydata.sorted_idx[part_id_sorted];
		...
		if (do_skip_align || do_skip_rotate)
		{
			...
			sampling.setOrientation(part_id_sorted - my_first_part_id, old_rot, old_tilt, old_psi);
		}
		else if (do_only_sample_tilt)
		{
			RFLOAT old_psi = DIRECT_A2D_ELEM(exp_metadata, metadata_offset, METADATA_PSI);
			sampling.psi_angles[part_id_sorted - my_first_part_id] = old_psi;
		}
		if (do_skip_align)
		{
			...
			sampling.setTranslation(part_id_sorted - my_first_part_id, rounded_offset_x * my_pixel_size, 
				rounded_offset_y * my_pixel_size, rounded_offset_z * my_pixel_size,
				(do_helical_refine) && (!ignore_helical_symmetry), rot_deg, tilt_deg, psi_deg); // clear for first particle
		}
	}
	...
	// We don't need to read file here, while the reading process is in globalThreadExpectationSomeParticles
	...
	if (!do_cpu)
	{
		...
		exp_ipart_ThreadTaskDistributor->resize(my_last_part_id - my_first_part_id + 1, XMIPP_MIN(4, my_last_part_id - my_first_part_id + 1));
		exp_ipart_ThreadTaskDistributor->reset();
		// printf("nr_threads= %d\n", nr_threads);
		// printf("block size = %d\n", XMIPP_MIN(4, my_last_part_id - my_first_part_id + 1));
		#pragma omp parallel for num_threads(nr_threads)
		for (int thread_id = 0; thread_id < nr_threads; thread_id++)
		{
			globalThreadExpectationSomeParticles(this, thread_id,task_limit);
		}
		// printf("Thread end %d %d\n",my_last_part_id,my_first_part_id);
		// fflush(stdout);
	...
	else
	{
		...
		// std::cout<<tbb::global_control::max_allowed_parallelism<<"xjldebug"<<std::endl;
		...
		tbb::parallel_for(my_first_part_id, my_last_part_id+1, [&](long int i) {
			...
			if (cpuOptimiser->thread_id == 0)
				timer.tic(TIMING_ESP_ONEPART);
			else if (cpuOptimiser->thread_id == nr_threads -1)
...
//#define DEBUG_GETALLDIFF2
	...
	if (exp_ipass==0)
    {
        std::cout<<"exp_Mcoarse_significant.clear();"<<std::endl;
...
//#define DEBUG_GETALLDIFF2
											...
											//if (ibody==1 && part_id == 0 && exp_ipass==0 && ihidden_over == 40217)
											{
												...
												std::cerr << " _over= " << ihidden_over << " diff2= " << diff2
														<< " rot= " << rrot
														<< " tilt= " << ttilt
														<< " psi= " << ppsi
...
//#define MAC_OSX
	...
	if (exp_ipass==0)
    {
        std::cout<<"exp_Mcoarse_significant.resize(exp_nr_images, XSIZE(exp_Mweight));"<<std::endl;
...
void MlOptimiser::updateAngularSampling(bool myverb)
{
	...
	else
	{
		...
		if (do_proceed_resolution && do_proceed_hidden_variables || grad_suspended_local_searches_iter >= 0)
		{
			...
			// Old rottilt step is already below 75% of estimated accuracy: have to stop refinement? step很小,下次的err会在0.25的now以下
