...
void AccProjectorPlan::setup(
		...
		Matrix2D<RFLOAT> &L_,
		Matrix2D<RFLOAT> &R_)//L和R记录的是body的rotation
{
	...
	for (long int idir = idir_min, iorient = 0; idir <= idir_max; idir++)
	{
		for (long int ipsi = ipsi_min, ipart = 0; ipsi <= ipsi_max; ipsi++, iorient++)
		{
			...
			// Get prior for this direction and skip calculation if prior==0，概率计算？
			...
			if (do_proceed)
			{
				...
				for (long int iover_rot = 0; iover_rot < nr_oversampled_rot; iover_rot++, ipart++)
				{
					if (sampling.is_3D)
					{
						alphas[orientation_num] = oversampled_rot[iover_rot];//angles，RFLOAT类型
	...
	eulers.resizeHostCopy(orientation_num * 9);//旋转矩阵
	eulers.deviceAlloc();
...
void AccProjectorPlanNew::setup(
		HealpixSampling &sampling,
		std::vector<RFLOAT> &directions_prior,
		std::vector<RFLOAT> &psi_prior,
		std::vector<int> &pointer_dir_nonzeroprior,
		...
		Matrix2D<RFLOAT> &R_)//L和R记录的是body的rotation
{
	...
	AccPtrNew<XFLOAT> alphas =  eulers.make<XFLOAT>(nr_dir * nr_psi * nr_oversampled_rot * 9);
	AccPtrNew<XFLOAT> betas =   eulers.make<XFLOAT>(nr_dir * nr_psi * nr_oversampled_rot * 9);
	AccPtrNew<XFLOAT> gammas =  eulers.make<XFLOAT>(nr_dir * nr_psi * nr_oversampled_rot * 9);
	AccPtrNew<XFLOAT> perturb = eulers.make<XFLOAT>((size_t)9);
	AccPtrNew<XFLOAT> adjustL = eulers.make<XFLOAT>((size_t)9);
	AccPtrNew<XFLOAT> adjustR = eulers.make<XFLOAT>((size_t)9);
	...
	// printf("0e0001a gamma");
	// gammas.print_detail();
	// fflush(stdout);
}


void AccProjectorPlanNew::clear()
{
	orientation_num = 0;
	eulers.freeIfSet();
	eulers.setSize(0);
...
void AccProjectorPlanNew::set_up_from_old_plan(AccProjectorPlan &old_plan)
{
