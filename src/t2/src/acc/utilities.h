...
template <typename T>
static std::pair<size_t, T> getArgMinOnDevice(AccPtrNew<T> &ptr)
{
#ifdef _CUDA_ENABLED
return CudaKernels::getArgMinOnDevice<T>(ptr);
#else
...
template <typename T>
static std::pair<size_t, T> getArgMaxOnDevice(AccPtrNew<T> &ptr)
{
#ifdef _CUDA_ENABLED
return CudaKernels::getArgMaxOnDevice<T>(ptr);
#else
...
template <typename T>
static int filterGreaterZeroOnDevice(AccPtrNew<T> &in, AccPtrNew<T> &out)
{
#ifdef _CUDA_ENABLED
CudaKernels::MoreThanCubOpt<T> moreThanOpt(0.);
	return CudaKernels::filterOnDevice(in, out, moreThanOpt);
...
#ifdef _CUDA_ENABLED
template< typename T>
void InitValue(AccPtr<T> &data, T value, size_t Size, cudaStream_t stream)
{
int grid_size = ceil((float)Size/(float)INIT_VALUE_BLOCK_SIZE);
	cuda_kernel_init_value<T><<< grid_size, INIT_VALUE_BLOCK_SIZE, 0, stream >>>(
			~data,
			value,
			Size,
...
static void check_eps(
		XFLOAT* d_benchmark_array,
		XFLOAT* d_test_array,
		unsigned int array_len,
		char*   array_name)
{
	XFLOAT* benchmark_array, * test_array;
	XFLOAT  max_relative_error = -1, max_relative_error_benchmark = -1, max_relative_error_test = -1;
	benchmark_array = (XFLOAT* )malloc(array_len * sizeof(XFLOAT));
	test_array      = (XFLOAT* )malloc(array_len * sizeof(XFLOAT));
	cudaMemcpy(benchmark_array, d_benchmark_array, array_len * sizeof(XFLOAT), cudaMemcpyDeviceToHost);
	cudaMemcpy(test_array     , d_test_array     , array_len * sizeof(XFLOAT), cudaMemcpyDeviceToHost);

	bool benchmark_all_zero = true;
	bool test_array_has_nan = false;
	for (int i = 0; i < array_len; i ++)
	{
		if (isnan(test_array[i]))
			test_array_has_nan = true;
		if (benchmark_array[i] != 0.0)
			benchmark_all_zero = false;
		
	}
	printf("array_len : %d\n", array_len);
	if (benchmark_all_zero)
		printf("### benchmark array all zero !\n");
	if (test_array_has_nan)
		printf("### test array has nan !\n");
	for (int i = 0; i < array_len; i ++) {
		if (fabs(benchmark_array[i]) != 0.f)
        {
            XFLOAT relative_error = fabs((test_array[i] - benchmark_array[i]) / benchmark_array[i]);
            if (relative_error > max_relative_error)
            {
                max_relative_error = relative_error;
				max_relative_error_benchmark = benchmark_array[i];
				max_relative_error_test = test_array[i];
            }
			// if(relative_error>0.001||isnan(test_array[i]))
			// if(i<20)
			// {
			// 	printf("### %s relative error : %e detail :  %e / %e  %d\n", array_name, relative_error, test_array[i], benchmark_array[i],i);
			// 	// exit(-1);
			// }
        }
	}
	
	printf("### %s max relative error : %e detail :  %e / %e\n", array_name, max_relative_error, max_relative_error_test, max_relative_error_benchmark);
...
//We only want as many blocks as there are chunks of orientations to be treated
	...
	// XFLOAT *g_wdiff2s_parts_f, *g_wdiff2s_AA_f, *g_wdiff2s_XA_f;
	
	// cudaMallocAsync((void**)&g_wdiff2s_parts_f, image_size * sizeof(XFLOAT), stream);
	// cudaMallocAsync((void**)&g_wdiff2s_AA_f,    image_size * sizeof(XFLOAT), stream);
	// cudaMallocAsync((void**)&g_wdiff2s_XA_f,    image_size * sizeof(XFLOAT), stream);

	// cudaMemcpyAsync(g_wdiff2s_parts_f, g_wdiff2s_parts, image_size * sizeof(XFLOAT), cudaMemcpyDeviceToDevice, stream);
	// cudaMemcpyAsync(g_wdiff2s_parts_f, g_wdiff2s_parts, image_size * sizeof(XFLOAT), cudaMemcpyDeviceToDevice, stream);
	// cudaMemcpyAsync(g_wdiff2s_parts_f, g_wdiff2s_parts, image_size * sizeof(XFLOAT), cudaMemcpyDeviceToDevice, stream);
	...
	// // Launch and time the second kernel
    // cudaEventRecord(start);

	// cuda_kernel_wavg_half<REFCTF,REF3D,DATA3D,block_sz*2><<<block_dim,block_sz*2,(3*block_sz*2+9)*sizeof(XFLOAT),stream>>>(
	// 	g_eulers,
	// 	projector,
	// 	image_size,
	// 	orientation_num,
    ...
    // 	std::cout << "Time for the first kernel: " << milliseconds << " ms" << std::endl;
    ...
    // 	milliseconds = 0;
    // 	cudaEventElapsedTime(&milliseconds, start, stop);
    // 	std::cout << "Time for the second kernel: " << milliseconds << " ms" << std::endl;


	// 	cudaStreamSynchronize(stream);

	// 	check_eps(g_wdiff2s_parts_f, g_wdiff2s_parts, image_size, "g_wdiff2s_parts");
#else
	if (DATA3D)
	{
		...
		)
{
	...
	// XFLOAT *g_diff2s_f;
	// int diff_size=grid_size*eulers_per_block*translation_num;
	// // cudaMallocAsync((void**)&g_diff2s_f, diff_size * sizeof(XFLOAT), stream);
	// // cudaMemcpyAsync(g_diff2s_f, g_diff2s, diff_size * sizeof(XFLOAT), cudaMemcpyDeviceToDevice, stream);
	// // cudaStreamSynchronize(stream);
	// cudaMalloc(&g_diff2s_f, diff_size * sizeof(XFLOAT));
	// cudaMemcpy(g_diff2s_f, g_diff2s, diff_size * sizeof(XFLOAT), cudaMemcpyDeviceToDevice);
	// // check_eps(g_diff2s_f, g_diff2s, diff_size, "g_diff2s");
    ...
    // cudaEventRecord(start);
	// cuda_kernel_diff2_coarse_half<REF3D, DATA3D, block_sz, eulers_per_block, prefetch_fraction>
	...
	// 	// XFLOAT A,B;
	// 	// cudaMemcpy(&A, &(g_diff2s_f[73158]), sizeof(XFLOAT), cudaMemcpyDeviceToHost);
	// 	// cudaMemcpy(&B, &(g_diff2s[73158]), sizeof(XFLOAT), cudaMemcpyDeviceToHost);
	// 	// std::cout<<diff_size<<"  xjldebug:"<< A <<" "<< B <<std::endl;
	// 	check_eps(g_diff2s_f, g_diff2s, diff_size, "g_diff2s");
...
void kernel_weights_exponent_coarse(
		unsigned long num_classes,
		AccPtrNew<T> &g_pdf_orientation,
		AccPtrNew<bool> &g_pdf_orientation_zeros,
		AccPtrNew<T> &g_pdf_offset,
		AccPtrNew<bool> &g_pdf_offset_zeros,
		AccPtrNew<T> &g_Mweight,
		T g_min_diff2,
		unsigned long nr_coarse_orient,
		unsigned long  nr_coarse_trans)
{
