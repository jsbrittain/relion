...
public:
	...
	void resize(unsigned _nr_images, unsigned long _part_id)
	{
		this->metadata_offset = 0;
		this->part_id = _part_id;
		power_img.resize(_nr_images);
		highres_Xi2_img.resize(_nr_images);
		Fimg.resize(_nr_images);
		Fimg_nomask.resize(_nr_images);
		Fctf.resize(_nr_images);
		old_offset.resize(_nr_images);
		prior.resize(_nr_images);
		max_index.resize(_nr_images);
		sum_weight_class.resize(_nr_images);
	};
};
...
public:
	...
	//coarse>significant的连续段记录(AccPtr是数组)，每个ori起始开始记（登记值是coarse>significant数组中的起始位置），记录trans连续的长度

	size_t firstPos, lastPos; // positions in indexedDataArray data and index arrays to slice out
	size_t weightNum, jobNum; // number of weights and jobs this class weightNum是jobExtent的求和，jobNum是job数组的长度
...
// class IndexedDataArrayNew: public IndexedDataArray
// {
// public:
// 	 IndexedDataArrayNew(AccPtrFactoryNew ptrFactory):
// 	 	IndexedDataArray((AccPtrFactory)ptrFactory)
// 	{};
// 	 IndexedDataArrayNew(IndexedDataArrayNew &parent, IndexedDataArrayMask &mask):
// 		IndexedDataArray(parent, mask)
// 	{};
// };
...
public:
	...
	AccPtrNew<size_t> rot_id, rot_idx, trans_idx, ihidden_overs;
	...
	void setDataSize(size_t newSize, AccPtrFactoryNew ptrFactory)
	{
		weights.~AccPtrNew(); 
		rot_id.~AccPtrNew();
		rot_idx.~AccPtrNew(); 
		trans_idx.~AccPtrNew();
		ihidden_overs.~AccPtrNew();	
		...
		trans_idx.setSize(newSize);
		ihidden_overs.setSize(newSize);
		// weights.getAllocator_task()->print_p();
	}
	void setDataSize(size_t newSize)
	{
		weights.setSize(newSize);
	...
	void print_size()
	{
		...
		// printf("\t\trot_id size: %d\n",rot_id.getSize());
		// printf("\t\trot_idx size: %d\n",rot_idx.getSize());
		// printf("\t\ttrans idx size %d\n",trans_idx.getSize());
		// printf("\t\tihidden_overs size %d\n",ihidden_overs.getSize());
		weights.print_detail();
		rot_id.print_detail();
		rot_idx.print_detail();
		trans_idx.print_detail();	
		ihidden_overs.print_detail();
	}
