...
// template <typename T>
// class ImageNew : public Image<T>
// {
// 	// ImageNew(AccPtrFactoryNew &f):
// 	// 	AccPtrNew<T>(f.make<T>()),
// 	// 	x(0), y(0), z(0), fourier(false)
// 	// {}
// };

template <typename T>
class ImageNew : public AccPtrNew<T>
{
private:
	int x,y,z;
	bool fourier; //Is this a Fourier space data array
...
public:
	...
	ImageNew(int xdim, AccPtrFactoryNew &f):
		AccPtrNew<T>(f.make<T>(xdim)),
		x(xdim), y(1), z(1), fourier(false)
	{}

	ImageNew(int xdim, int ydim, AccPtrFactoryNew &f):
		AccPtrNew<T>(f.make<T>(xdim*ydim)),
		x(xdim), y(ydim), z(1), fourier(false)
	{}

	ImageNew(int xdim, int ydim, int zdim, AccPtrFactoryNew &f):
		AccPtrNew<T>(f.make<T>(xdim*ydim*zdim)),
		x(xdim), y(ydim), z(zdim), fourier(false)
	{}

	template <typename T1>
	ImageNew(MultidimArray<T1> img, AccPtrFactoryNew &f):
		AccPtrNew<T>(f.make<T>(img.nzyxdim)),
		x(img.xdim), y(img.ydim), z(img.zdim), fourier(false)
	{}

	ImageNew(int box_dim, bool is_fourier, bool is3D, AccPtrFactoryNew &f)
	{
		setSize(box_dim, is_fourier, is3D);
		AccPtrNew<T>(f.make<T>(x*y*z));
	}
	...
	void setHost(MultidimArray<T1> &img)
	{
		if (img.xdim != x || img.ydim != y || img.zdim != z)
		{
			if (img.nzyxdim > AccPtrNew<T>::getSize())
			{
				AccPtrNew<T>::freeIfSet();
				setSize(img);
				AccPtrNew<T>::hostAlloc();
			}
			else
				setSize(img);
		}

		if (AccPtrNew<T>::getHostPtr() == NULL)
			AccPtrNew<T>::hostAlloc();

		T *ptr = AccPtrNew<T>::getHostPtr();
