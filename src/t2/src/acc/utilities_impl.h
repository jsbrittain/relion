		...
		bool is3D)
{
    ...
    // Make a holder for the spectral profile and copy to the GPU
    // AccDataTypes::Image<XFLOAT> NoiseSpectra(sigmaNoiseSpectra, ptrFactory);
    AccPtrNew<XFLOAT> NoiseSpectra = RandomImage.make<XFLOAT>(sigmaNoiseSpectra.nzyxdim);
    NoiseSpectra.allAlloc();
    ...
    // Set up states to seeda and run randomization on the GPU
    // AccDataTypes::Image<curandState > RandomStates(RND_BLOCK_NUM*RND_BLOCK_SIZE,ptrFactory);
    AccPtrNew<curandState> RandomStates = RandomImage.make<curandState>(RND_BLOCK_NUM*RND_BLOCK_SIZE);
    RandomStates.deviceAlloc();
...
{
	//Temporary array because translate is out-of-place
	AccPtrNew<XFLOAT> temp = img_out.make<XFLOAT>(img_in.nzyxdim);
	temp.allAlloc();
...
template <class MlClass>
void normalizeAndTransformImage(	AccPtrNew<XFLOAT> &img_in,
									MultidimArray<Complex > &img_out,
									MlClass *accMLO,
									size_t xSize,
									size_t ySize,
									size_t zSize)
{
			...
			AccPtrNew<ACCCOMPLEX> d_Fimg = img_in.make<ACCCOMPLEX>(xSize * ySize * zSize);
			d_Fimg.allAlloc();
			accMLO->transformer1.fouriers.streamSync();
			windowFourierTransform2(
					accMLO->transformer1.fouriers,
...
static void softMaskBackgroundValue(
	AccDataTypes::ImageNew<XFLOAT> &vol,
	XFLOAT   radius,
	XFLOAT   radius_p,
	XFLOAT   cosine_width,
	AccPtrNew<XFLOAT> &g_sum,
	AccPtrNew<XFLOAT> &g_sum_bg)
{
	int block_dim = 128; //TODO: set balanced (hardware-dep?)
#ifdef _CUDA_ENABLED
		cuda_kernel_softMaskBackgroundValue<<<block_dim,SOFTMASK_BLOCK_SIZE,0, vol.getStream()>>>(
...
static void cosineFilter(
		AccDataTypes::ImageNew<XFLOAT> &vol,
		bool do_Mnoise,
		AccDataTypes::ImageNew<XFLOAT> Noise,
		XFLOAT radius,
		XFLOAT radius_p,
		XFLOAT cosine_width,
		XFLOAT sum_bg_total)
