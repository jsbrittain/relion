		...
		XFLOAT part_scale)
{
	...
	// if (bid == 0 && tid == 0) {
	// 	__half tmpa = __float2half(s_eulers[0]);
	// 	__half tmpb = __float2half(s_eulers[1]);
	// 	__half tmpc = tmpa + tmpb;
	// 	float tmpc_float = __half2float(tmpc);
	// 	printf("cuda_kernel_wavg tmpc : %f + %f = %f\n", s_eulers[0], s_eulers[1], tmpc_float);
	// 	printf("tmpa : %f  tmpb : %f\n", __half2float(tmpa), __half2float(tmpb));
	// }
	// __syncthreads();
...
#ifdef _CUDA_HALF
template<bool REFCTF, bool REF3D, bool DATA3D, int block_sz>
__global__ void cuda_kernel_wavg_half(
		XFLOAT *g_eulers,
		AccProjectorKernel projector,
		unsigned image_size,
		unsigned long orientation_num,
		...
		XFLOAT part_scale)
{
	CUDA_HALF ref_real, ref_imag, img_real, img_imag, trans_real, trans_imag;
		...
		if(pixel<image_size)
		{
			...
			for (unsigned long itrans = 0; itrans < translation_num; itrans++)
			{
				...
				if (weight >= significant_weight)
				{
					...
					CUDA_HALF diff_real = ref_real - trans_real;
					CUDA_HALF diff_imag = ref_imag - trans_imag;

					s_wdiff2s_parts[tid] += weight * (XFLOAT)(diff_real*diff_real + diff_imag*diff_imag);

					s_sumXA[tid] +=  weight * (XFLOAT)( ref_real * trans_real + ref_imag * trans_imag);
					s_sumA2[tid] +=  weight * (XFLOAT)( ref_real*ref_real  +  ref_imag*ref_imag );
				}
			}
