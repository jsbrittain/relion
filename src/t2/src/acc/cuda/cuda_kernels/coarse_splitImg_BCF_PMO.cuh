#ifndef COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_BCF_PMO_CUH
#define COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_BCF_PMO_CUH
...
__global__ void CoarseMatrixKernelIm2colSplitImgBCFProjOverlapKernel(
	XFLOAT *g_eulers,
	XFLOAT *trans_x,
	XFLOAT *trans_y,
	XFLOAT *g_real,
...
struct CoarseMatrixKernelIm2colSplitImgBCFProjOverlap {
  const int translation_num_;
  const int orientation_num_;
  const int image_size_;
  const int sm_num_;

  CoarseMatrixKernelIm2colSplitImgBCFProjOverlap(
...
//   }
  ...
  void run(XFLOAT *g_eulers, 
	...
	CoarseScheduler<TParams::kTransBlockSize,
      ...
      CoarseMatrixKernelIm2colSplitImgBCFProjOverlapKernel<TParams>
	  <<<grid, block, 0, stream>>>(
	...
	// 	CoarseMatrixKernelIm2colSplitImgBCFProjOverlapKernel<SelectedTParams><<<grid, block, 0, stream>>>(
    //         g_eulers, trans_x, trans_y, g_real, g_imag, projector, g_corr,
    //         g_diff2s, g_diff2s_dest, translation_num_, orientation_num_, image_size_, g_coor_xy);
    //   });
    // }
...
#endif // COARSE_MATRIX_KERNEL_IM2COL_SPLITIMG_BCF_PMO_CUH