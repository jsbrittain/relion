...
__device__ inline void pixel_index2coor_fine(int idx, int imgX, int imgY, int maxR, int &x, int &y) {
	x = idx % imgX;
	y = idx / imgX;
	if (y > maxR) {
		if (y >= imgY - maxR) {
...
{
CUDA_HALF s, c;
CUDA_HALF theta = __hmul(x, tx) + __hmul(y, ty);
s = hsin(theta);
c = hcos(theta);

tReal = __hmul(c, real) - __hmul(s, imag);
tImag = __hmul(c, imag) + __hmul(s, real);
}
__device__ __forceinline__ void translatePixel(
	int x,
	int y,
...
{
CUDA_HALF s, c;
CUDA_HALF theta = __hmul(x, tx) + __hmul(y, ty) + __hmul(z, tz);
s = hsin(theta);
c = hcos(theta);
