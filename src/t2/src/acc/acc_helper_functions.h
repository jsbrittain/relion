...
long int makeJobsForDiff2Fine(
		...
		int chunk);
long int makeJobsForDiff2Fine1(
		OptimisationParamters &op,  SamplingParameters &sp,
		long int orientation_num, long int translation_num,
		ProjectionParams &FineProjectionData,
		std::vector< long unsigned > &iover_transes,
		std::vector< long unsigned > &ihiddens,
		long int nr_over_orient, long int nr_over_trans, int img_id,
		IndexedDataArrayNew &FPW, // FPW=FinePassWeights
		IndexedDataArrayMask &dataMask,
		int chunk);
...
long int  makeJobsForCollect1(IndexedDataArrayNew &FPW,
        IndexedDataArrayMask &dataMask,
        unsigned long NewJobNum); // FPW=FinePassWeights
/*
...
template< typename T>
void deviceInitValue(AccPtr<T> &data, T value, size_t Size, cudaStream_t stream)
{
	AccUtilities::InitValue<T>(data, value, Size, stream);
}
...
template< typename T>
void arrayOverThreshold(AccPtrNew<T> &data, AccPtrNew<bool> &passed, T threshold)
{
#ifdef _CUDA_ENABLED
	int grid_size = ceil((float)data.getSize()/(float)OVER_THRESHOLD_BLOCK_SIZE);
	cuda_kernel_array_over_threshold<T><<< grid_size, OVER_THRESHOLD_BLOCK_SIZE, 0, data.getStream() >>>(
...
template< typename T>
size_t findThresholdIdxInCumulativeSum(AccPtrNew<T> &data, T threshold)
{
	int grid_size = ceil((float)(data.getSize()-1)/(float)FIND_IN_CUMULATIVE_BLOCK_SIZE);
	if(grid_size==0)
	{
	...
	else
	{
		// return (0);
#ifdef _CUDA_ENABLED
		AccPtrNew<size_t >  idx(1, data.getStream(), data.getAllocator_task(),data.getAllocator());
		idx[0] = 0;

		idx.putOnDevice();
			// cudaError_t errorStatus;
			// idx.streamSync();
			// LAUNCH_PRIVATE_ERROR(cudaGetLastError(),errorStatus);
		cuda_kernel_find_threshold_idx_in_cumulative<<< grid_size, FIND_IN_CUMULATIVE_BLOCK_SIZE, 0, data.getStream() >>>(
				~data,
				threshold,
				data.getSize()-1,
