...
long int makeJobsForDiff2Fine(
		...
		long int orientation_num, long int translation_num, // 这里的orientation_num, translation_num都是细采样后的总数
		...
		int chunk)
{
	...
	for (long unsigned i = 0; i < orientation_num; i++)
	{
		...
		for (long unsigned j = 0; j < translation_num; j++)
		{
			...
			long int ihidden = FineProjectionData.iorientclasses[i] * sp.nr_trans + ihiddens[j];//coarse矩阵中该角度的index
