...
public:
    ...
    void coreAllocate()
    {
        ...
        {
            #ifdef CUDA_ENABLED
                pin_alloc((void **)&data, sizeof(T) * nzyxdim);
            #else
     ...
     * @endcode
     */
    //return -1:->normal; return x:should be coreAllocate();
    int resizeNoCp_Mcoarse(long int Ndim, long int Zdim, long int Ydim, long int Xdim)
    {
        if (Ndim*Zdim*Ydim*Xdim == nzyxdimAlloc && data != NULL)
        {
            std::cout<<"Ndim*Zdim*Ydim*Xdim == nzyxdimAlloc && data != NULL"<<std::endl;
            return -2;
        }

        if (Xdim <= 0 || Ydim <= 0 || Zdim <= 0 || Ndim <= 0)
        {
            std::cout<<"Xdim <= 0 || Ydim <= 0 || Zdim <= 0 || Ndim <= 0"<<std::endl;
            clear();
            return -2;
        }
    ...
    /** Resize to a given size for Mcoarse_significant
     *
     */
    void resizeNoCp(long int Ndim, long int Zdim, long int Ydim, long int Xdim)
    {
        ...
        try
        {
            ...
            {
                #ifdef CUDA_ENABLED
                    pin_alloc((void **)&new_data, sizeof(T) * NZYXdim);
                #else
                    new_data = (T*)RELION_ALIGNED_MALLOC(sizeof(T) * NZYXdim);
                #endif
            }
    ...
    void resize(long int Ndim, long int Zdim, long int Ydim, long int Xdim)
    {
        ...
        {
            printf("Allocate: No space left\n");fflush(stdout);
            REPORT_ERROR( "Allocate: No space left");
        }
        // printf("beforeresize 0cx006\n");fflush(stdout);
    ...
    {
        // printf("beforeresize 0cy001\n");fflush(stdout);

        if (NSIZE(*this) != NSIZE(v) || XSIZE(*this) != XSIZE(v) ||
            YSIZE(*this) != YSIZE(v) || ZSIZE(*this) != ZSIZE(v) || data==NULL)
            resize(NSIZE(v), ZSIZE(v), YSIZE(v), XSIZE(v));
        // printf("beforeresize 0cy002\n");fflush(stdout);

        STARTINGX(*this) = STARTINGX(v);
        STARTINGY(*this) = STARTINGY(v);
        STARTINGZ(*this) = STARTINGZ(v);
        // printf("endresize 0cy003\n");fflush(stdout);
...
#undef MAX
    ...
    {
        // printf("beforememcpy 0c0000\n");fflush(stdout);

        if (&op1 != this)
        {
            // printf("beforememcpy 0c0001\n");fflush(stdout);

            if (data == NULL || !sameShape(op1))
                resize(op1);
            // printf("beforememcpy 0c0002\n");fflush(stdout);
            memcpy(data,op1.data,MULTIDIM_SIZE(op1)*sizeof(T));
            // printf("aftermemcpy 0c0003\n");fflush(stdout);
