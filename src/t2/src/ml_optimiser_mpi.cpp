...
(which in all tested cases is perfectly fine), then you could use an box-size of ~"  << sLowBoxLim << "." << std::endl;
	...
	std::cout<<"xjldebug="<<mydata.MDimg.numberOfObjects()<<std::endl;
...
#define JOB_NPAR  (JOB_LAST - JOB_FIRST + 1)
	...
	if (do_gpu && ! node->isLeader())
	{
		...
		for (int i = 0; i < accDataBundles.size(); i ++)
		{
			...
			// printf("INFO: Device %d of rank %d has %zu MB total and %zu MB free %d %d\n", ((MlDeviceBundle*)accDataBundles[i])->device_id, node->rank, total/1024/1024, free/1024/1024,required_free/1024/1024,allocationSize/1024/1024);
			// printf("INFO:cudaDeviceShares%d %d\n",cudaDeviceShares[i],i);
	...
	MPI_Barrier(node->followerC);
		for (int i = 0; i < accDataBundles.size(); i ++)
			// ((MlDeviceBundle*)accDataBundles[i])->setupTunableSizedObjects(allocationSizes[i],threadcountOnDevice[i]);
#ifdef NEWMEM
			((MlDeviceBundle*)accDataBundles[i])->setupTunableSizedObjects(allocationSizes[i],threadcountOnDevice[i]);
	...
	if (do_cpu  && ! node->isLeader())
	{
		...
		for (int iclass = 0; iclass < nr_classes; iclass++)
		{
			...
			std::cout<<mdlX<<" "<<mdlY<<mdlZ<<std::endl;
		...
		{
			int mdlX = b->backprojectors[j].mdlX;
			int mdlY = b->backprojectors[j].mdlY;
			std::cout<<mdlX<<" "<<mdlY<<std::endl;
		}
	}  // do_cpu
#endif // ALTCPU
	/************************************************************************/
	...
	if (node->isLeader())
	{
		...
		try
		{
			...
			if(xjl_type==1)
			{
			my_last_particle_halfset1=my_first_particle_halfset1+1000;//xjldebug0106
			my_last_particle_halfset2=my_first_particle_halfset2+1000;//xjldebug0106
			progress_bar_step_size = XMIPP_MAX(1, 1006 / 60);
			std::cout << " 10006 particles " << iter << std::endl;
			std::cout << my_last_particle_halfset1 << std::endl;
			}
...
//#define DEBUG_MPIEXP2
				...
				if (do_split_random_halves)
				{
					...
					if (random_halfset == 1)
					{
						...
						if(process_num==1) JOB_LAST=my_last_particle_halfset1;
						else{
							int chunk_size = (nr_particles_todo - my_nr_particles_done + 2 * process_num - 1) / (2 * process_num);
							chunk_size = XMIPP_MAX(nr_pool, chunk_size);
							// chunk_size = min(chunk_size, remaining);
							JOB_LAST  = XMIPP_MIN(my_last_particle_halfset1, JOB_FIRST + chunk_size - 1);
							// std::cout << "###";
							// std::cout << JOB_FIRST << "  "  << JOB_LAST << "  " << nr_particles_todo << "  " <<  my_nr_particles_done << std::endl;
						}
					}
					else
					{
						...
						if(process_num==1) JOB_LAST=my_last_particle_halfset2;
						else
						{
							int chunk_size = (nr_particles_todo - my_nr_particles_done + 2 * process_num - 1) / (2 * process_num);
							chunk_size = XMIPP_MAX(nr_pool, chunk_size);
							JOB_LAST  = XMIPP_MIN(my_last_particle_halfset2, JOB_FIRST + chunk_size - 1);
						}
					}
				}
				else
...
void MlOptimiserMpi::iterate()
{
	...
	for (iter = iter + 1; iter <= nr_iter; iter++)
    {
...
#ifdef DEBUG
		std::cerr << " before combineAllWeightedSums..."<< combine_weights_thru_disc<< std::endl;
#endif
		...
		// for(int xjl=1;xjl<node->size;xjl++)
		// {
		// 	if(node->rank ==xjl)
		// 	{
		// 		std::cout<<"%%%%%%%%%%%nodeid:"<<node->rank<<std::endl;
		// 		timer.printTimes(false);
		// 	}
		// 	MPI_Barrier(MPI_COMM_WORLD);
		// }
